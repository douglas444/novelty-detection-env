...
import higia.Clusters.ClusteringBla;
import higia.Clusters.SummClusters;
import higia.utils.*;
...
public class kNN_kem extends AbstractClassifier implements MultiClassClassifier {
	...
	public void setModelContext(InstancesHeader context) {
		try {
			...
			try {
				File file = new File("higia_output.txt");
				File file2 = new File("higia_centers.txt");
				if (file.exists())
					file.delete();
				if (file2.exists())
					file2.delete();
...
//		System.out.println("windowkernel " + this.windowKernel.size());
		...
		if (this.neWindow.size() < minNovelty) {
			MicroCluster micro = new MicroCluster(in, "", "anormal", timestamp, null);
				...
				if (sim == 0) {
					...
					for (int i = 0; i < this.neWindow.size(); i++) {
						if (this.neWindow.get(i).getRelevanceStamp() < threshold) {
							MicroCluster element = new MicroCluster(in, "", "anormal", timestamp, null);
				...
				for (int i = 0; i < this.neWindow.size(); i++) {
					...
					if (this.neWindow.get(i).getN() >= 10) {
						...
						if (minDistance <= (this.windowKernel.get(indiceCluster).getRadius() * threshold)) {
							...
							this.neWindow.get(i).setConceptCategory(this.windowKernel.get(indiceCluster).getConceptCategory());
						} else {
							classes.add(Double.toString(newClass));
							this.neWindow.get(i).setType("novelty");
							this.neWindow.get(i).setConceptCategory(ConceptCategory.NOVELTY);
						}
						...
						final List<ClusterSummary> knownClustersSummaries = this.windowKernel
								.stream()
								.filter(microCluster -> microCluster.getConceptCategory() == ConceptCategory.KNOWN)
						...
						final ConceptClassificationContext context = new ConceptClassificationContext()
								...
								.setDecision(this.neWindow.get(i).getConceptCategory());
