...
public class Miner implements OptionHandler{
        ...
        if(totalUnComitted > this.Minpts && (minst.Id > this.Minpts + LastCheck)) // then check for novel classes
        {
            ...
            if(L > 1)
            {
                ...
                for(int i = 0; i < outkm.Clusters.length; i ++) {
                    ...
                    final ConceptClassificationContext context = fillContext(ConceptCategory.NOVELTY,
                            en, outkm.Clusters[i], i, outkm.m_data, null);
    ...
    private static ConceptClassificationContext fillContext(final ConceptCategory decision,
                                                            final Classifier[] en, final Cluster cluster,
                                                            final int index, final Minstances minstances,
        ...
        final List<Sample> targetSamples;
        final double[] targetCentroidAttributes = cluster.centroid.toDoubleArray();
        final double[] targetCentroidX = Arrays.copyOfRange(targetCentroidAttributes, 0, targetCentroidAttributes.length - 1);
        final Integer targetCentroidY = (int) targetCentroidX[targetCentroidX.length - 1];
        final Sample targetClusterCentroid = new Sample(targetCentroidX, targetCentroidY);
        final double targetClusterStandardDeviation = cluster.sumx2 / (double) cluster.n - cluster.meand * cluster.meand;

        targetClusterSummary = new PseudoPoint(targetClusterCentroid, targetClusterStandardDeviation);

        knownClusterSummaries = Arrays.stream(en)
                .map(classifier -> (ReascCtrl) classifier)
                .map(reascCtrl -> Arrays.asList(reascCtrl.getClusters()))
                .flatMap(Collection::stream)
                .map(c -> {
                    final double[] attributes = c.centroid.toDoubleArray();
                    final double[] x = Arrays.copyOfRange(attributes, 0, attributes.length - 1);
                    final Integer y = (int) attributes[attributes.length - 1];
                    final double standardDeviation = c.sumx2 / (double) c.n - c.meand * c.meand;
                    return new PseudoPoint(new Sample(x, y), standardDeviation);
                })
        ...
        if (datapoints == null) {
            ...
            targetSamples = Arrays
                    .stream(minstances.getM_Instances())
                    .map(o -> (Minstance) o)
                    .filter(minstance -> minstance.ClusterId == index)
                    .map(m -> {
                        final double[] attributes = m.toDoubleArray();
                        final double[] x = Arrays.copyOfRange(attributes, 0, attributes.length - 1);
                        final Integer y = (int) attributes[attributes.length - 1];
            ...
            targetSamples = datapoints.stream()
                    .filter(datapoint -> datapoint.getClusterId() == index)
                    .map(m -> {
                        final double[] attributes = m.avector;
                        final double[] x = Arrays.copyOfRange(attributes, 0, attributes.length - 1);
                        final Integer y = (int) attributes[attributes.length - 1];
                        return new Sample(x, y);
                    })
                ...
                if(/*trainData.numInstances() == this.ChunkSize*/estimatedCP != -1)
                {
                    ...
                    */
                    ...
                    for (int i = 0; i < ((ReascCtrl) C).mcClusters.length; ++i) {
                        ...
                        final ConceptClassificationContext context = fillContext(ConceptCategory.KNOWN,
                                En, mcCluster, i, null, ((ReascCtrl) C).data);
                        interceptor.NOVELTY_DETECTION_AL_FRAMEWORK.with(context).executeOrDefault(() -> {});
                    }
